...
const LENS_ADDR = '0x73f52bd9e59edbdf5cf0dd59126cef00ecc31528';
const CRV_CAULDRON_ADDR = '0x207763511da879a900973A5E092382117C3c1588';
const CRV_CAULDRON_2_ADDR = '0x7d8dF3E4D06B0e19960c19Ee673c0823BEB90815';
...
export class RefundCalculator {
  ...
  constructor(cauldronAddr: string, borrowAddr: string, votingAddr: string, blockNumber?: number) {
    this.cauldronAddr = cauldronAddr;
    ...
    this.lensContract = new ethers.Contract(LENS_ADDR, lensAbi, this.provider);
  ...
  async getBorrowAmount(): Promise<BigNumber> {
    const userPosition = await this.lensContract.getUserPosition(this.cauldronAddr, this.borrowAddr, {
      blockTag: this.blockNumber,
    });
    return userPosition.borrowValue;
  }
  ...
  async maxWeeklyRefund(): Promise<BigNumber> {
    const maxRefund = (await this.getBorrowAmount()).mul(MAX_REFUND_RATE).div(10000).div(WEEKS_IN_YEAR);
    return maxRefund;
  }
...
async function main() {
  ...
  console.log('The closest block was: ', blockNumber);
  const spellPrice = await calc.getSpellPrice();
  console.log('SPELL price ($):', formatBn(spellPrice, 18, 6));
  console.log('Total borrow amount ($):', formatBn(await calc.getBorrowAmount(), 18, 2));
